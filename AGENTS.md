...
- **Anonymous bound data** only via `bind X { scope:public }`, never for the auth entity. `requires:` matches exact route patterns. When several patterns match, the static one wins (`/projects/new` over `/projects/:id`).
...
- **Sessions.** HttpOnly cookie only (no JS-readable token). CSRF gate for cookie-authenticated mutations (`Host` only; `X-Forwarded-Host` iff the peer is in `CRONUS_TRUSTED_PROXIES`). HTML login/register are native `POST` forms (`/login`, `/register`); JSON stays at `/api/auth/*`. Logout that clears the cookie is `POST /logout` or `POST /api/auth/logout`; `GET /logout` only redirects to `/login`. `JWT_SECRET` and existing `.cronus/jwt.key` / `.cronus/webhook.key` must be ≥ 32 bytes; generated key files and SQLite/audit files are 0600.
- **HTTP.** Binds `127.0.0.1` unless `--host`/`CRONUS_HOST`. `--prod`/`CRONUS_ENV=production` 404s internal routes (`/zeus`, `/api/_context`, `/docs*`, `GET /graphql` playground, `/.cronus/version`, …) and sends HSTS. Body limit is 1 MiB. Rate limit keys on the socket peer (`/api/*`, `POST /graphql`, `/_form`, `/_action`; stricter for login/signup and `/hooks`). `CRONUS_CORS_ORIGIN=*` is ignored.
...
- **Webhooks.** `http://` only; private targets blocked unless `CRONUS_WEBHOOK_ALLOW_PRIVATE=1` (IPv6 also blocks 6to4 `2002::/16` and `2001:2::/48`). Inbound `POST /hooks/*` requires the same HMAC as outbound (`X-Cronus-Timestamp` + `X-Cronus-Signature`, 300s skew).
...
- `index` / `searchable` create SQLite indexes; every table gets `idx_{table}__owner_id`. Migrate `ALTER TABLE ADD COLUMN` for new fields. HMR re-parses and swaps live `AppState` then bumps `/.cronus/version`.
- Request handlers time out after `CRONUS_HANDLER_TIMEOUT_SECS` (default 30) with 504. GraphQL selections deeper than 8 or with more than 200 nodes are `COST_EXCEEDED`.
- `cronus seed` / `deploy` / `test` / `parse` load `*.cronus` via `parser::load_cwd()` (same union as `run`).
