...
- **Sessions.** HttpOnly cookie only (no JS-readable token). CSRF gate for cookie-authenticated mutations (`Host` only; `X-Forwarded-Host` iff the peer is in `CRONUS_TRUSTED_PROXIES`). Logout that clears the cookie is `POST /api/auth/logout`; `GET /logout` only redirects to `/login`. `JWT_SECRET` and existing `.cronus/jwt.key` / `.cronus/webhook.key` must be ≥ 32 bytes; generated key files and SQLite/audit files are 0600.
