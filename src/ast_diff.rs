...
pub enum AstChange {
    ...
    EntitySharedChanged { name: String, now_shared: bool },
    FieldAdded { entity: String, field: String, field_type: String },
    FieldRemoved { entity: String, field: String },
    FieldTypeChanged { entity: String, field: String, old_type: String, new_type: String },
    PageAdded { route: String },
    PageRemoved { route: String },
    ApiAdded { prefix: String },
    ApiRemoved { prefix: String },
    ApiRouteAdded { prefix: String, method: String, path: String },
    ApiRouteRemoved { prefix: String, method: String, path: String },
    WebhookAdded { entity: String },
    WebhookRemoved { entity: String },
...
impl AstChange {
    ...
    pub fn describe(&self) -> String {
        match self {
            AstChange::EntityAdded { name } =>
                format!("  + Entity '{}' added", name),
            AstChange::EntityRemoved { name } =>
                format!("  - Entity '{}' removed", name),
            AstChange::EntitySharedChanged { name, now_shared } =>
                format!("  ~ Entity '{}' shared = {}", name, now_shared),
            AstChange::FieldAdded { entity, field, field_type } =>
                format!("  + {}.{} ({}) added", entity, field, field_type),
            AstChange::FieldRemoved { entity, field } =>
                format!("  - {}.{} removed", entity, field),
            AstChange::FieldTypeChanged { entity, field, old_type, new_type } =>
                format!("  ~ {}.{} type: {} -> {}", entity, field, old_type, new_type),
            AstChange::PageAdded { route } =>
                format!("  + Page '{}' added", route),
            AstChange::PageRemoved { route } =>
                format!("  - Page '{}' removed", route),
            AstChange::ApiAdded { prefix } =>
                format!("  + API '{}' added", prefix),
            AstChange::ApiRemoved { prefix } =>
                format!("  - API '{}' removed", prefix),
            AstChange::ApiRouteAdded { prefix, method, path } =>
                format!("  + API route {} {} in '{}'", method, path, prefix),
            AstChange::ApiRouteRemoved { prefix, method, path } =>
                format!("  - API route {} {} in '{}'", method, path, prefix),
            AstChange::WebhookAdded { entity } =>
                format!("  + Webhook for '{}' added", entity),
            AstChange::WebhookRemoved { entity } =>
                format!("  - Webhook for '{}' removed", entity),
            AstChange::StyleChanged { key, old_value, new_value } =>
...
pub fn snapshot_from_ast(nodes: &[AstNode]) -> AstSnapshot {
        ...
        match node {
            AstNode::Entity(e) => {
                entities.push(SnapshotEntity {
                    name: e.name.clone(),
                    shared: e.shared,
                    fields: e.fields.iter().map(|f| SnapshotField {
                        name: f.name.clone(),
            ...
            AstNode::Api(a) => {
                apis.push(SnapshotApi {
                    ...
                    routes: a.routes.iter().map(|r| SnapshotRoute {
            ...
            AstNode::Style(s) => {
                if let Some(ref v) = s.theme { style.insert("theme".to_string(), v.clone()); }
                if let Some(ref v) = s.accent { style.insert("accent".to_string(), v.clone()); }
                if let Some(ref v) = s.radius { style.insert("radius".to_string(), v.clone()); }
                if let Some(ref v) = s.font { style.insert("font".to_string(), v.clone()); }
                for (k, v) in &s.config {
    ...
    AstSnapshot { entities, pages, apis, webhooks, style }
}
...
fn diff_entities(old: &AstSnapshot, new: &AstSnapshot, changes: &mut Vec<AstChange>) {
    let old_map: HashMap<&str, &SnapshotEntity> = old.entities.iter()
        .map(|e| (e.name.as_str(), e)).collect();
    let new_map: HashMap<&str, &SnapshotEntity> = new.entities.iter()
        .map(|e| (e.name.as_str(), e)).collect();

    // Removed entities
    for name in old_map.keys() {
        if !new_map.contains_key(name) {
            changes.push(AstChange::EntityRemoved { name: name.to_string() });
        }
    }
    ...
    for (name, new_ent) in &new_map {
        match old_map.get(name) {
            None => {
                changes.push(AstChange::EntityAdded { name: name.to_string() });
            }
            Some(old_ent) => {
                // shared changed?
                ...
                // field diffs
                let old_fields: HashMap<&str, &SnapshotField> = old_ent.fields.iter()
                    .map(|f| (f.name.as_str(), f)).collect();
                let new_fields: HashMap<&str, &SnapshotField> = new_ent.fields.iter()
                    .map(|f| (f.name.as_str(), f)).collect();
...
fn diff_pages(old: &AstSnapshot, new: &AstSnapshot, changes: &mut Vec<AstChange>) {
    for route in &old.pages {
        if !new.pages.contains(route) {
            changes.push(AstChange::PageRemoved { route: route.clone() });
        }
    }
    for route in &new.pages {
        if !old.pages.contains(route) {
            changes.push(AstChange::PageAdded { route: route.clone() });
        }
    }
}

fn diff_apis(old: &AstSnapshot, new: &AstSnapshot, changes: &mut Vec<AstChange>) {
    let old_map: HashMap<&str, &SnapshotApi> = old.apis.iter()
        .map(|a| (a.prefix.as_str(), a)).collect();
    let new_map: HashMap<&str, &SnapshotApi> = new.apis.iter()
        .map(|a| (a.prefix.as_str(), a)).collect();

    for prefix in old_map.keys() {
        if !new_map.contains_key(prefix) {
            changes.push(AstChange::ApiRemoved { prefix: prefix.to_string() });
        }
    }
    ...
    for (prefix, new_api) in &new_map {
        match old_map.get(prefix) {
            None => {
                changes.push(AstChange::ApiAdded { prefix: prefix.to_string() });
            }
            Some(old_api) => {
                // Compare routes within this API
                let old_routes: Vec<(&str, &str)> = old_api.routes.iter()
                    .map(|r| (r.method.as_str(), r.path.as_str())).collect();
                let new_routes: Vec<(&str, &str)> = new_api.routes.iter()
                    .map(|r| (r.method.as_str(), r.path.as_str())).collect();
...
fn diff_webhooks(old: &AstSnapshot, new: &AstSnapshot, changes: &mut Vec<AstChange>) {
    for entity in &old.webhooks {
        if !new.webhooks.contains(entity) {
            changes.push(AstChange::WebhookRemoved { entity: entity.clone() });
        }
    }
    for entity in &new.webhooks {
        if !old.webhooks.contains(entity) {
            changes.push(AstChange::WebhookAdded { entity: entity.clone() });
        }
    }
}
