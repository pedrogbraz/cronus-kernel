...
mod tests {
    ...
    fn skips_chart_figure_stub() {
        ...
        assert_ne!(html, area);
        let chart = crate::cronus_ui_widgets::render(
            &crate::cronus_ui_widgets::test_stub("heatmap-chart"),
        )
        .unwrap();
        assert!(chart.contains("<figure data-slot=\"heatmap-chart\""));
        reject_stub(&html);
    }
    ...
    fn no_voodoo_even_when_runtime_on() {
        crate::voodoo::with_enabled(true, || {
            let html = render(&stub("heatmap", "Activity"));
            reject_stub(&html);
            assert!(html.contains("data-slot=\"heatmap\""));
            assert!(html.contains("data-slot=\"heatmap-day\""));
        });
    }
    ...
    fn chrome_is_token_only() {
        ...
        assert!(css.contains("[data-slot=\"heatmap\"]"));
