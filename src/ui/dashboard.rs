...
use crate::parser::{ComponentItemNode, ComponentNode, SectionNode};
...
pub fn render_dashboard_page(
    ...
    let sidebar_comp = components
        .iter()
        .find(|c| c.layout.as_deref() == Some("sidebar"));
    let topbar_comp = components.iter().find(|c| {
        c.layout.as_deref() == Some("inline")
            && c.style
                .as_deref()
                .map(|s| s.contains("topbar"))
                .unwrap_or(false)
    });
    ...
    let quick_links = sections
        .iter()
        .find(|s| s.section_type == "quick-links" || s.section_type == "links");
...
pub(super) fn build_dashboard_sidebar(
    comp: Option<&ComponentNode>,
    section: Option<&SectionNode>,
    current_route: &str,
) -> String {
    let mut brand = "";
    let mut subtitle = "";
        ...
        for item in &items {
            ...
            let is_active = (!href.is_empty() && href == current_route)
                || item
                    ...
                    .map(|s| s == "true")
                    .unwrap_or(false);
            let is_bottom = bottom_types.contains(&icon)
                || title.eq_ignore_ascii_case("support")
                || title.eq_ignore_ascii_case("docs");

            let nav_idx = if is_bottom {
                bottom_items_html.matches("<a ").count()
            } else {
                nav_items_html.matches("<a ").count()
            };
  ...
  <span>{title}</span>
        ...
        for item in &sec.items {
            ...
            let nav_idx2 = if position == "bottom" {
                bottom_items_html.matches("<a ").count()
            } else {
                nav_items_html.matches("<a ").count()
            };
...
pub(super) fn build_dashboard_topbar(comp: Option<&ComponentNode>) -> String {
    let search_placeholder = comp
        .and_then(|c| {
            c.items
                .iter()
                .find(|i| i.config.get("icon").map(|s| s == "search").unwrap_or(false))
                .map(|i| i.text.as_str())
        })
        .unwrap_or("Search documentation...");

    let avatar_url = comp
        .and_then(|c| c.props.get("avatar").map(|s| s.as_str()))
        .unwrap_or("");
...
pub(super) fn build_api_keys_card(section: Option<&SectionNode>, mode: &str) -> String {
    ...
    for item in &sec.items {
        ...
        if is_masked {
            ...
            let reveal_text = if description.starts_with("sk_test") {
                "Hide"
            } else {
                "Reveal"
            };
...
pub fn render_billing_dashboard(
    ...
    let payment_methods = sections
        .iter()
        .find(|s| s.section_type == "payment-methods");
    let recent_invoices = sections
        .iter()
        .find(|s| s.section_type == "recent-invoices");
...
pub(super) fn build_billing_current_plan(section: Option<&SectionNode>) -> String {
    ...
    for item in &sec.items {
        ...
        let value_html = if !suffix.is_empty() {
            ...
            format!(
                r#"<span style="font-weight:600">{value}</span>"#,
                value = value
            )
        };
...
pub(super) fn build_billing_usage_status(section: Option<&SectionNode>) -> String {
    ...
    let action_item = sec
        ...
        .iter()
        .find(|i| i.get("_type").map(|s| s.as_str()) == Some("action"));
    let link_text = action_item
        .and_then(|i| i.get("title"))
        .map(|s| s.as_str())
        .or_else(|| sec.config.get("link_text").map(|s| s.as_str()))
        .unwrap_or("");
    let link_href = action_item
...
pub(super) fn build_billing_payment_methods(section: Option<&SectionNode>) -> String {
    ...
    for item in &sec.items {
        ...
        let icon = item.get("icon").map(|s| s.as_str()).unwrap_or("");
        let badge_html = if badge_type == "dark"
            || (!badge_type.is_empty() && badge_type != "light")
        {
        ...
        let desc_html = if !meta.is_empty()
            && (status == "default" || meta.to_lowercase().contains("default"))
        {
            format!(
            ...
            format!(
                r#"<div style="font-size:12px;color:#5e5e5e">{}</div>"#,
                description
            )
        } else {
            String::new()
    ...
    for item in &sec.items {
        ...
        if !text.is_empty() {
            let icon_html = if !icon.is_empty() {
                format!(
                    r#"<span class="material-symbols-outlined" style="font-size:14px;color:#5e5e5e">{}</span>"#,
                    icon
                )
            } else {
                String::new()
...
pub(super) fn build_payouts_history(section: Option<&SectionNode>) -> String {
    ...
    let actions: Vec<&std::collections::HashMap<String, String>> = sec
        .items
        .iter()
    ...
    for action in &actions {
        ...
        let icon_html = if !icon.is_empty() {
            format!(
                r#"<span class="material-symbols-outlined" style="font-size:14px">{}</span>"#,
                icon
            )
        } else {
            String::new()
...
pub fn render_unified_dashboard(
    ...
    let balance_cols = balance
        .and_then(|s| s.config.get("cols"))
        .map(|s| s.as_str())
        .unwrap_or("8");
    let balance_html = build_payouts_balance_card(balance);

    let usage_cols = usage
        .and_then(|s| s.config.get("cols"))
        .map(|s| s.as_str())
        .unwrap_or("4");
    ...
    let history_cols = history
        .and_then(|s| s.config.get("cols"))
        .map(|s| s.as_str())
        .unwrap_or("8");
    let history_html = build_payouts_history(history);

    let invoices_cols = invoices
        .and_then(|s| s.config.get("cols"))
        .map(|s| s.as_str())
        .unwrap_or("4");
...
pub fn render_payment_links_dashboard(
    ...
    let product_grids: Vec<&SectionNode> = sections
        .iter()
...
pub(super) fn build_payment_links_page_header(section: Option<&SectionNode>) -> String {
    ...
    let subtitle_html = if !subtitle.is_empty() {
        format!(
            r#"<p style="color:#5e5e5e;font-size:16px;font-weight:500;margin:0">{}</p>"#,
            subtitle
        )
    } else {
        String::new()
...
pub(super) fn build_payment_links_promo(section: Option<&SectionNode>) -> String {
    ...
    let badge_html = if !badge_text.is_empty() {
        format!(
            r#"<span style="display:inline-block;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;padding:4px 12px;border-radius:999px;background:rgba(255,255,255,0.2);color:#fff;margin-bottom:16px">{}</span>"#,
            badge_text
        )
    } else {
        String::new()
    ...
    let subtitle_html = if !subtitle.is_empty() {
        format!(
            r#"<p style="font-size:14px;color:#a1a1aa;max-width:480px;line-height:1.6;margin:0 0 32px">{}</p>"#,
            subtitle
        )
    } else {
        String::new()
    ...
    let right_html = if !image_url.is_empty() {
        format!(
            r#"<div style="flex:1;position:relative;min-height:200px"><img src="{}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;border-radius:8px;opacity:0.6" alt=""></div>"#,
            image_url
        )
    } else {
...
pub(super) fn build_payment_links_info_bar(section: Option<&SectionNode>) -> String {
    ...
    let title_html = if !title.is_empty() {
        format!(
            r#"<p style="font-size:14px;font-weight:700;letter-spacing:-0.02em;color:#1a1c1c;margin:0">{}</p>"#,
            title
        )
    } else {
        String::new()
    ...
    let subtitle_html = if !subtitle.is_empty() {
        format!(
            r#"<p style="font-size:12px;color:#5e5e5e;margin:0">{}</p>"#,
            subtitle
        )
    } else {
        String::new()
...
pub fn render_checkout_dashboard(
    ...
    let product_summary = sections
        .iter()
        .find(|s| s.section_type == "product-summary");
    let trust_indicators = sections
        .iter()
        .find(|s| s.section_type == "trust-indicators");
...
pub fn render_security_dashboard(
    ...
    let security_status = sections
        .iter()
        .find(|s| s.section_type == "security-status");
    let security_policies = sections
        .iter()
        .find(|s| s.section_type == "security-policies");
...
pub(super) fn build_security_sidebar(
    comp: Option<&ComponentNode>,
    section: Option<&SectionNode>,
) -> String {
...
pub(super) fn build_security_page_header(section: Option<&SectionNode>) -> String {
    ...
    let subtitle_html = if !subtitle.is_empty() {
        format!(
            r#"<p style="color:#71717a;font-size:14px;margin:0">{}</p>"#,
            subtitle
        )
    } else {
        String::new()
...
pub(super) fn build_security_team_members(section: Option<&SectionNode>) -> String {
    ...
    let badge_html = if !badge.is_empty() {
        format!(
            r#"<span style="background:#e8e8e8;font-size:12px;padding:4px 10px;border-radius:999px;font-weight:500">{}</span>"#,
            badge
        )
    } else {
        String::new()
...
pub(super) fn build_security_2fa_status(section: Option<&SectionNode>) -> String {
    ...
    let compliance_item = sec
        .items
        .iter()
        .find(|i| i.get("_type").map(|s| s.as_str()) != Some("action"));
    let compliance_label = compliance_item
        .and_then(|i| i.get("title"))
        .map(|s| s.as_str())
        .unwrap_or("");
    let compliance_value = compliance_item
...
pub fn render_settings_dashboard(
    ...
    let sidebar_html = if let Some(sec) = sidebar_section {
        render_sidebar(sec)
    } else {
        String::new()
    };
    let topbar_html = if let Some(sec) = topbar_section {
        build_settings_topbar(sec)
    } else {
        String::new()
    };
    ...
    let header_html = if let Some(sec) = page_header {
        ...
        let subtitle = sec.subtitle.as_deref().unwrap_or("");
        format!(
            r#"<header style="margin-bottom:48px">
  ...
  <input type="{input_type}" value="{value}" style="width:100%;background:#0e0e0e;border:0.5px solid rgba(76,69,70,0.3);border-radius:8px;padding:10px 16px;color:#e2e2e2;font-size:14px;font-family:inherit;outline:none;transition:box-shadow 0.2s" onfocus="this.style.boxShadow='0 0 0 2px rgba(173,198,255,0.1)'" onblur="this.style.boxShadow='none'">
        ...
        let note_html = if note.is_empty() {
            ...
            format!(
                r#"<p style="font-size:12px;color:rgba(226,226,226,0.7);font-style:italic;margin:0">{}</p>"#,
                note
            )
        };
        format!(
            r#"<section style="background:#1f1f1f;border-radius:12px;overflow:hidden;border:0.5px solid rgba(76,69,70,0.15)">
  ...
  <div style="background:#1b1b1b;padding:16px 32px;display:flex;justify-content:space-between;align-items:center">
    ...
    let api_keys_html = if let Some(sec) = api_keys {
        ...
        for (i, item) in sec.items.iter().enumerate() {
            ...
            let divider = if i > 0 {
                r#"<div style="height:0.5px;background:rgba(76,69,70,0.1)"></div>"#
            } else {
                ""
            };
  ...
  <div class="key-actions" style="display:flex;gap:8px;opacity:0;transition:opacity 0.15s">
    ...
    <button style="padding:8px;background:none;border:none;cursor:pointer;color:rgba(207,196,197,1)" onmouseover="this.style.color='#ffb4ab'" onmouseout="this.style.color='rgba(207,196,197,1)'"><span class="material-symbols-outlined" style="font-size:16px">delete</span></button>
        ...
        format!(
            r#"<section style="margin-top:48px">
  ...
  <button style="width:100%;padding:10px;border-radius:8px;border:1px solid rgba(76,69,70,1);background:transparent;color:#e2e2e2;font-size:14px;font-weight:500;cursor:pointer;transition:background 0.15s;font-family:inherit" onmouseover="this.style.background='#353535'" onmouseout="this.style.background='transparent'">{action}</button>
        ...
        format!(
            r#"<section style="display:grid;grid-template-columns:repeat(2,1fr);gap:24px;margin-top:48px">{cards}</section>"#,
            cards = cards_html
        )
    } else {
        String::new()
    ...
    let subscription_html = if let Some(sec) = subscription {
        ...
        for item in &sec.items {
            ...
            let icon = item.get("icon").map(|s| s.as_str()).unwrap_or("");
            if item.get("badge").is_some() {
                price = t;
                billing_note = item.get("subtitle").map(|s| s.as_str()).unwrap_or("");
  ...
  {title}
        ...
        format!(
            r#"<section style="background:rgba(31,31,31,0.4);backdrop-filter:blur(32px);border:0.5px solid rgba(76,69,70,0.2);padding:32px;border-radius:12px;position:relative;overflow:hidden">
  ...
  <button style="background:none;border:none;cursor:pointer;color:rgba(207,196,197,0.4);transition:color 0.15s" onmouseover="this.style.color='#e2e2e2'" onmouseout="this.style.color='rgba(207,196,197,0.4)'"><span class="material-symbols-outlined" style="font-size:18px">download</span></button>
        ...
        format!(
            r#"<section style="background:#0e0e0e;border:0.5px solid rgba(76,69,70,0.15);border-radius:12px;overflow:hidden;margin-top:32px">
  ...
  <button style="width:100%;padding:12px;font-size:10px;text-transform:uppercase;font-weight:700;letter-spacing:0.15em;color:rgba(207,196,197,1);background:#1b1b1b;border:none;cursor:pointer;transition:color 0.15s;font-family:inherit" onmouseover="this.style.color='#e2e2e2'" onmouseout="this.style.color='rgba(207,196,197,1)'">{inv_footer}</button>
    ...
    let support_html = if let Some(sec) = support {
        ...
        let action_label = sec
            ...
            .map(|s| s.as_str())
            .unwrap_or("");
        let action_html = if action_label.is_empty() {
            String::new()
        } else {
            format!(
                r##"<a href="#" style="font-size:12px;color:#adc6ff;font-weight:700;text-decoration:none;display:flex;align-items:center;gap:4px" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">
    {label}
    <span class="material-symbols-outlined" style="font-size:14px">arrow_outward</span>
        ...
        format!(
            r#"<section style="background:#1b1b1b;padding:24px;border-radius:12px;border-left:4px solid rgba(173,198,255,0.4);margin-top:32px">
  ...
  {action}
    ...
    let danger_html = if let Some(sec) = danger {
        ...
        let desc = sec
            ...
            .unwrap_or("");
        format!(
            r#"<div style="margin-top:96px;border-top:1px solid rgba(255,180,171,0.2);padding-top:48px">
...
pub(super) fn build_settings_topbar(section: &SectionNode) -> String {
    ...
    let avatar_item = section
        .items
        .iter()
        .find(|i| i.get("_type").map(|s| s.as_str()) == Some("image"));
    let avatar_src = avatar_item
        .and_then(|i| i.get("src"))
        .map(|s| s.as_str())
        .unwrap_or("");
    let avatar_alt = avatar_item
        .and_then(|i| i.get("title"))
        .map(|s| s.as_str())
        .unwrap_or("");
    let avatar_html = if !avatar_src.is_empty() {
        format!(
            r#"<div style="height:32px;width:32px;border-radius:50%;background:#2a2a2a;border:0.5px solid rgba(76,69,70,0.2);overflow:hidden">
...
pub fn render_order_detail_dashboard(
    ...
    let price_breakdown = sections
        .iter()
        .find(|s| s.section_type == "price-breakdown");
    let payment_info = sections.iter().find(|s| s.section_type == "payment-info");
    let customer_profile = sections
        .iter()
        .find(|s| s.section_type == "customer-profile");
    let shipping_timeline = sections
        .iter()
        .find(|s| s.section_type == "shipping-timeline");
    ...
    let header_html = if let Some(sec) = order_header {
        ...
        let back_link = sec
            ...
            .unwrap_or("");
        let badge = sec.config.get("badge").map(|s| s.as_str()).unwrap_or("");
        let action_primary = sec
            .config
            .get("action_primary")
            .map(|s| s.as_str())
            .unwrap_or("");
        let action_secondary = sec
            .config
            .get("action_secondary")
            .map(|s| s.as_str())
            .unwrap_or("");
        let back_html = if back_link.is_empty() {
            String::new()
        } else {
            format!(
                r##"<a href="#" style="color:#adc6ff;font-size:14px;font-weight:500;text-decoration:none;display:flex;align-items:center;gap:4px"><span class="material-symbols-outlined" style="font-size:14px">arrow_back</span> {}</a>"##,
                back_link
            )
        };
        let badge_html = if badge.is_empty() {
            ...
            format!(
                r#"<span style="padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;background:#3630bf;color:#e2dfff;letter-spacing:0.1em;text-transform:uppercase">{}</span>"#,
                badge
            )
        };
        let sec_btn = if action_secondary.is_empty() {
            String::new()
        } else {
            format!(
                r#"<button style="padding:10px 24px;background:#2a2a2a;color:#e2e2e2;font-weight:600;border-radius:8px;border:0.5px solid rgba(76,69,70,0.2);cursor:pointer;font-family:inherit;font-size:14px;transition:background 0.15s" onmouseover="this.style.background='#353535'" onmouseout="this.style.background='#2a2a2a'">{}</button>"#,
                action_secondary
            )
        };
        let pri_btn = if action_primary.is_empty() {
            String::new()
        } else {
            format!(
                r#"<button style="padding:10px 24px;background:linear-gradient(135deg,#adc6ff 0%,#c2c1ff 50%,#e9b3ff 100%);color:#000;font-weight:800;border-radius:8px;border:none;cursor:pointer;font-family:inherit;font-size:14px;transition:all 0.15s;box-shadow:0 4px 16px rgba(173,198,255,0.2)" onmousedown="this.style.transform='scale(0.95)'" onmouseup="this.style.transform='scale(1)'">{}</button>"#,
                action_primary
            )
        };
        format!(
            r#"<section style="display:flex;justify-content:space-between;align-items:flex-end">
  <div>
    ...
    let items_html = if let Some(sec) = line_items {
        ...
        for item in &sec.items {
            ...
            let img_html = if image.is_empty() {
                ...
                format!(
                    r#"<div style="width:96px;height:96px;border-radius:8px;overflow:hidden;background:#0e0e0e;border:0.5px solid rgba(76,69,70,0.2);flex-shrink:0">
  ...
  <div style="text-align:right">
    ...
    <p style="font-size:12px;color:rgba(207,196,197,1);margin:2px 0 0">Qty: {qty}</p>
        ...
        format!(
            r#"<div style="background:#1f1f1f;border-radius:12px;padding:32px;border:0.5px solid rgba(76,69,70,0.1)">
  ...
  <div style="display:flex;flex-direction:column;gap:16px">{rows}</div>"#,
    ...
    let breakdown_html = if let Some(sec) = price_breakdown {
        ...
        for item in &sec.items {
            ...
            if is_total {
                total_html = format!(
                    r#"<div style="display:flex;justify-content:space-between;align-items:center;padding-top:16px">
  ...
  <span>{label}</span><span style="font-family:monospace">{value}</span>
        ...
        format!(
            r#"  <div style="margin-top:48px;padding-top:32px;border-top:1px solid rgba(76,69,70,0.1);display:flex;flex-direction:column;gap:16px">
    {rows}
    {total}
  </div>
</div>"#,
    ...
    let payment_html = if let Some(sec) = payment_info {
        ...
        for (i, item) in sec.items.iter().enumerate() {
            ...
            let icon_html = if !label_text.is_empty() {
                format!(
                    r#"<div style="width:48px;height:32px;border-radius:4px;background:#0e0e0e;border:1px solid rgba(76,69,70,0.3);display:flex;align-items:center;justify-content:center">
  <span style="font-size:10px;font-weight:900;letter-spacing:0.1em;color:#e2e2e2">{}</span>
                ...
                format!(
                    r#"<span class="material-symbols-outlined" style="color:#a944dc;font-size:24px">{}</span>"#,
                    icon
                )
            } else {
                String::new()
  ...
  <div style="display:flex;align-items:center;gap:16px">
    ...
    <div>
      ...
      <p style="font-size:12px;color:rgba(207,196,197,1);margin:2px 0 0">{detail_sub}</p>
        ...
        format!(
            r#"<div style="display:grid;grid-template-columns:1fr 1fr;gap:24px">{}</div>"#,
            cards
        )
    } else {
        String::new()
    ...
    let customer_html = if let Some(sec) = customer_profile {
        ...
        let avatar_html = if avatar.is_empty() {
            ...
            format!(
                r#"<div style="height:56px;width:56px;border-radius:50%;background:#2a2a2a;border:1px solid rgba(76,69,70,0.3);overflow:hidden;flex-shrink:0">
  <img src="{}" alt="{}" style="width:100%;height:100%;object-fit:cover">
        ...
        format!(
            r#"<div style="background:#1f1f1f;border-radius:12px;padding:24px;border:0.5px solid rgba(76,69,70,0.1)">
    ...
    let timeline_html = if let Some(sec) = shipping_timeline {
        ...
        for item in &sec.items {
            ...
            let (dot_style, title_color) = match style {
                "future" => (
                    "background:#0e0e0e;border:1px solid rgba(76,69,70,0.5)",
                    "color:rgba(226,226,226,0.4)",
                ),
                "active" => (
                    "background:#1f1f1f;border:2px solid #adc6ff;animation:pulse 2s infinite",
                    "color:#adc6ff",
                ),
            ...
            let detail_html = if detail.is_empty() {
                ...
                format!(
                    r#"<p style="font-size:11px;margin:4px 0 0;color:rgba(207,196,197,1);font-style:italic">{}</p>"#,
                    detail
                )
            };
            let sub_color = if style == "future" {
                "color:rgba(226,226,226,0.4)"
            } else {
                "color:rgba(207,196,197,1)"
            };
  ...
  <p style="font-size:10px;margin:8px 0 0;font-weight:700;color:#c2c1ff">{meta}</p>
        ...
        format!(
            r#"<div style="background:#0e0e0e;border-radius:12px;padding:24px;border:0.5px dashed rgba(76,69,70,0.15)">
